...
SpeedTracker.prototype._getRemoteFile = function (file) {
  ...
  return this.options.remote.repos.getContents({
    ...
    //var content = new Buffer.from(JSON.stringify(response.data.content), 'base64').toString();
    // var content = JSON.stringify(response.content)

    var content = base64.decode(response.data.content)

    //console.log('RES', typeof(response), response.data.content, content, response.data.download_url)
    //fetch(response.data.download_url).then(res => res.text()).then(body => console.log(body))
...
SpeedTracker.prototype._saveTest = function (profile, content, isScheduled) {
  ...
  return this._getRemoteFile(path).then(data => {
    try {
      ...
      // console.log('PAYLOAD', payload, 'data sha', data.sha, 'DATA', data)

      // return this.options.remote.api.repos.updateFile({
      return this.options.remote.repos.updateFile({
        // user: this.options.user,
        owner: this.options.user,
        repo: this.options.repo,
        ...
        content: base64.encode(JSON.stringify(payload)),
        message: message
      })
    } catch (err) {
    ...
    if (err.status === 404) {
      ...
      return this.options.remote.repos.createFile({
        // user: this.options.user,
        owner: this.options.user,
        repo: this.options.repo,
        branch: this.options.branch,
      ...
      return Promise.reject(Utils.buildError('CORRUPT_RESULT_FILE_2'))
    }
  })
}
...
SpeedTracker.prototype.getConfig = function (force) {
  ...
  return this._getRemoteFile('speedtracker.yml').then(data => {
    ...
    //console.log('err getremotefile', err)
    return Promise.reject(Utils.buildError('INVALID_CONFIG'))
  })
}
